...
pub fn init(gpa: std.mem.Allocator, cmd: *Command, ls_cmd: *Command, pull_cmd: *Command, push_cmd: *Command) void {
    ...
    cmd.* = Command{ .use = "image", .aliases = &.{ "images", "img" }, .short = "Manage images",
...
fn lsFn(_: *Command, _: [][]const u8) void { var b: [256]u8 = undefined; var ow = std.Io.File.Writer.init(std.Io.File.stdout(), io(), &b); const w = &ow.interface; w.print("REPOSITORY   TAG       SIZE\n", .{}) catch {}; ow.flush() catch {}; }
fn pullFn(_: *Command, args: [][]const u8) void { var b: [256]u8 = undefined; var ow = std.Io.File.Writer.init(std.Io.File.stdout(), io(), &b); const w = &ow.interface; w.print("Pulling '{s}'...\n", .{args[0]}) catch {}; ow.flush() catch {}; }
fn pushFn(_: *Command, args: [][]const u8) void { var b: [256]u8 = undefined; var ow = std.Io.File.Writer.init(std.Io.File.stdout(), io(), &b); const w = &ow.interface; w.print("Pushing '{s}'...\n", .{args[0]}) catch {}; ow.flush() catch {}; }
