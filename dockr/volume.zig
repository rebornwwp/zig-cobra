...
fn lsFn(_: *Command, _: [][]const u8) void { var b: [256]u8 = undefined; var ow = std.Io.File.Writer.init(std.Io.File.stdout(), io(), &b); const w = &ow.interface; w.print("VOLUME NAME\n", .{}) catch {}; ow.flush() catch {}; }
fn createFn(_: *Command, args: [][]const u8) void { var b: [256]u8 = undefined; var ow = std.Io.File.Writer.init(std.Io.File.stdout(), io(), &b); const w = &ow.interface; w.print("Created volume '{s}'\n", .{args[0]}) catch {}; ow.flush() catch {}; }
