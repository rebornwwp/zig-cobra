...
pub fn main(init: std.process.Init) !void {
    ...
    var root_flags = pflag.FlagSet.init(gpa, "dockr"); defer root_flags.deinit();
    root_flags.stringVarP(&state.config, "config", "c", "/etc/dockr/config.toml", "config file path") catch {};
    var root_cmd = Command{ .use = "dockr", .short = "A self-sufficient runtime for containers",
        .long = "dockr is a CLI for managing containers, images, and volumes.\nUse 'dockr COMMAND --help' for more information on a command.",
        .flags = &root_flags, .persistent_pre_run = rootPersistentPreRun };
    root_cmd.iflags = @ptrCast(@alignCast(&state));

    var ctr_cmd: Command = undefined;
    var cr_cmd: Command = undefined; var cr_flags: pflag.FlagSet = undefined; defer cr_flags.deinit(); var cr_opts = types.ContainerRunOpts{};
    var cls_cmd: Command = undefined; var cls_flags: pflag.FlagSet = undefined; defer cls_flags.deinit();
    var cstart_cmd: Command = undefined;
    var cstop_cmd: Command = undefined;
    var crm_cmd: Command = undefined; var crm_flags: pflag.FlagSet = undefined; defer crm_flags.deinit();
    ...
    ctr_cmd = Command{ .use = "container", .aliases = &.{"containers"}, .short = "Manage containers",
    ...
    var ils_cmd: Command = undefined; var ipull_cmd: Command = undefined; var ipush_cmd: Command = undefined;
    img.init(gpa, &img_cmd, &ils_cmd, &ipull_cmd, &ipush_cmd);

    var vol_cmd: Command = undefined;
    var vls_cmd: Command = undefined; var vcreate_cmd: Command = undefined;
